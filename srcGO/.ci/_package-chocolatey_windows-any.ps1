...
function PACKAGE-Assemble-CHOCOLATEY-Content {
	...
	$___source = "${env:PROJECT_PATH_ROOT}\${env:PROJECT_GO}\"
	$___dest = "${_directory}\Data\${env:PROJECT_GO}\"
	$null = I18N-Assemble "${___source}" "${___dest}"
	$null = FS-Make-Directory "${___dest}"
	$___process = FS-Copy-All "${___source}" "${___dest}"
	if ($___process -ne 0) {
	...
	$___source = "${env:PROJECT_PATH_ROOT}\${env:PROJECT_GO}\.ci\"
	$___dest = "${_directory}\Data\${env:PROJECT_GO}\.ci\"
	$null = I18N-Assemble "${___source}" "${___dest}"
	$null = FS-Make-Directory "${___dest}"
	$___process = FS-Copy-All "${___source}" "${___dest}"
	if ($___process -ne 0) {
